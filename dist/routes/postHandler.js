...
async function postHandler(req, res) {
  ...
  `
    ...
    //      - The third parameter is an object that should contain the \`data\` field, passing all the parameters the function received (i.e., \`value\`).
    //   9. **Response Formatting**: All responses generated by the system must be formatted with line breaks (\`\\n\`) for better readability. This includes separating sections of the response such as code blocks or explanations.
    ...
    //      - The third parameter is an object that should contain the \`data\` field, passing all the parameters the function received (i.e., \`value\`).
    //   9. **Response Formatting**: All responses generated by the system must be formatted with line breaks (\`\\n\`) for better readability. This includes separating sections of the response such as code blocks or explanations.
    //   10. **Error Handling with Try-Catch**: All functions generated must be wrapped in a \`try-catch\` block to handle potential errors during execution. Inside the \`catch\` block, log the error using \`util.systemError()\`, following this structure:
    //       \`\`\`
    //       try {
    //         // Your code here
    //       } catch (error) {
    //         await util.systemError(error.message, 'functionName', { data: value });
    //         throw error;
    //       }
    //       \`\`\`
    //       This ensures that errors are caught and logged, without affecting the main flow of the function.
    //   Below is the relevant context retrieved by the RAG system, which may or may not be helpful in answering the user’s request:
    //   ${
    //     isRagWorking
    //       ? `${retrievedContext}`
    ...
    //   {
    //       "thinking": "Brief explanation of your reasoning to solve the problem",
    //       "response": "(async () => { try { let {parameter1, parameter2} = value; // Your code here } catch (error) { await util.systemError(error.message, 'functionName', { data: value }); throw error; } })();", // The function as a string with try-catch
    //       "user_mood": "positive|neutral|negative|curious|frustrated|confused",
    //       "suggested_questions": ["Question 1?", "Question 2?", "Question 3?"],
    //       "debug": {
    //         "context_used": true
